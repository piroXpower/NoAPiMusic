...
async def _watcher_vc(_, m: types.Message):
    await ArchonMusic.stop(m.chat.id)
            ...
            try:
                ...
                if remaining <= 30:
                    ...
                    if next and not next.file_path:
                        # Try the fast streamable-URL path first (near-
                        # instant) and only fall back to a full download
                        # if the API didn't return valid media for it —
